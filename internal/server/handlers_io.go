...
func (s *Server) apiImport(w http.ResponseWriter, r *http.Request) {
	...
	for i := range items {
		...
		if err := s.st.CreateEnvironment(r.Context(), &e); err != nil {
			s.importResult(w, r, len(names), names, err)
			return
		}
	...
	s.importResult(w, r, len(names), names, nil)
}

func (s *Server) importResult(w http.ResponseWriter, r *http.Request, n int, names []string, err error) {
	if wantsHTML(r) {
		if err != nil {
			s.render(w, "env_import.html", map[string]any{"Error": err.Error()})
			return
		}
...
// importPayload returns the raw JSON, whether posted as a form field (paste/upload
// page) or as a raw JSON body (API).
...
func (s *Server) pageImport(w http.ResponseWriter, r *http.Request) {
	s.render(w, "env_import.html", map[string]any{})
}
