...
func (s *Server) apiUpdateEnv(w http.ResponseWriter, r *http.Request) {
	...
	// Credentials are never returned by the API (MarshalJSON blanks them), so a
	// read-modify-write client (e.g. edp-ui's edit form) sends them back empty.
	// Treat empty as "unchanged" and keep the stored secret rather than wiping it;
	// a non-empty value replaces it. (e currently holds the decrypted secrets.)
	prevGitToken, prevRegPw := e.GitToken, e.RegistryPassword
	if err := bindEnv(r, e); err != nil {
		writeJSON(w, 400, map[string]string{"error": err.Error()})
		return
	}
	if e.GitToken == "" {
		e.GitToken = prevGitToken
	}
	if e.RegistryPassword == "" {
		e.RegistryPassword = prevRegPw
	}
