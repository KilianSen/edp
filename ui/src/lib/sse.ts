...
// EventSource cannot) and therefore works cross-origin with a Bearer token.
//
// Returns a function that aborts the stream. Default events arrive via onMessage;
// named events (e.g. edp's "done") arrive via onEvent(name, data).
export function streamSSE(path: string, handlers: StreamHandlers): () => void {
  ...
  (async () => {
    try {
      ...
      // One SSE event = consecutive non-blank lines; a blank line dispatches it.
      let eventName = "";
